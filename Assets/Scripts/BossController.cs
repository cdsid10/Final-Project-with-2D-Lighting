     ...
     if(actionCounter > 0)
        {
            ...
            if(actions[currentAction].shouldMove)
            {
                ...
                {
                    moveDir = actions[currentAction].pointToMoveTo.position - transform.position;
                }
            }


            theRB.velocity = moveDir * actions[currentAction].moveSpeed;
            ...
            if(actions[currentAction].shouldShoot)
            {
                ...
                if(shotCounter <= 0)
                {
                    shotCounter = actions[currentAction].timeBwShots;
